...
Commands:
  mongoback export  Export your database with a powered mongoexport

Cli options
  --askUri, -u          If the connection uri will be asked to the user. If
                        false, the uri deducted by the other options will be
                        used.
           [boolean] [default: By default it is asked if the option --uri is not
                                                                      specified]
  --askCollections, -c  If the collections to be exported will be asked to the
                        user. If false, the ones deducted by the other options
                        will be used.                  [boolean] [default: true]
  --askDestination, -d  If the outDir will be asked to the user. If false, the
                        one deducted by the other options will be used.
                                                       [boolean] [default: true]

Main connection options
  --uri            The uri of the MongoDB connection. If it is specified, the
                   other main connection options are ignored and (if not
                   explicitly specified) the askUri will be set to false[string]
  --host           The host of the MongoDB connection.
                                                 [string] [default: "localhost"]
  --port           The port of the MongoDB connection. [number] [default: 27017]
  --username       The username of the MongoDB connection.              [string]
  --password       The password of the MongoDB connection.              [string]
  --authDatabase   The authentication database of the MongoDB connection.
                                                                        [string]
  --authMechanism  The authentication mechanism of the MongoDB connection.
                                                                        [string]
  --srv            If the MongoDB connection uri is an srv.
                                                       [string] [default: false]

Advanced connection options
  --replicaSetName               The replicaSetName of the MongoDB connection.
                                                                        [string]
  --ssl                          If the MongoDB connection uses ssl or tls.
                                                      [boolean] [default: false]
  --sslCAFile                    Specifies the .pem file that contains both the
                                 TLS/SSL certificate and key.           [string]
  --sslPEMKeyFile                Specify the file name of the .pem file using
                                 relative or absolute paths.            [string]
  --sslPEMKeyPassword            Specifies the password to de-crypt the
                                 certificate-key file (i.e. --sslPEMKeyFile).
                                                                        [string]
  --sslCRLFile                   Specifies the .pem file that contains the
                                 Certificate Revocation List.           [string]
  --sslFIPSMode                  Directs the mongoexport to use the FIPS mode of
                                 the installed OpenSSL library. Note: deprecated
                                 option of mongoexport.
                                                      [boolean] [default: false]
  --sslAllowInvalidCertificates  Bypasses the validation checks for server
                                 certificates and allows the  use of invalid
                                 certificates.        [boolean] [default: false]
  --sslAllowInvalidHostnames     Disables the validation of the hostnames in
                                 TLS/SSL certificates.[boolean] [default: false]
  --gssapiServiceName            Specify the hostname of a service using
                                 GSSAPI/Kerberos. Only required if the hostname
                                 of a machine does not match the hostname
                                 resolved by DNS.                       [string]
  --gssapiHostName               Specify the hostname of a service using
                                 GSSAPI/Kerberos. Only required  if the hostname
                                 of a machine does not match the hostname
                                 resolved by DNS.                       [string]
  --slaveOk                      Sets the Read Preference to nearest, allowing
                                 mongoexport to read data from secondary replica
                                 set members. Note: Deprecated option of
                                 mongoexport.         [boolean] [default: false]
  --readPreference               Specify the read preference for mongoexport. It
                                 can be a string such as 'primary' or
                                 'secondary' or an object (config file only). If
                                 you want to pass the json object as a string,
                                 you must manually include it in apixes.[string]
  --journal                      Allows mongoexport operations to access the
                                 durability journal to ensure that the export is
                                 in a valid state. Note: Deprecated option of
                                 mongoexport.         [boolean] [default: false]
  --ipv6                         Enables IPv6 support that allows mongoexport to
                                 connect to the MongoDB instance using an IPv6
                                 network. Note: Deprecated on mongoexport.
                                                      [boolean] [default: false]
  --dbpath                       Specifies the directory of the MongoDB data
                                 files. Note: Deprecated on mongoexport [string]
  --directoryperdb               Use the --directoryperdb in conjunction with
                                 the corresponding option mongod, which allows
                                 mongoexport to export data from MongoDB that
                                 have every database’s files saved in discrete
                                 directories on disk. Note: Deprecated on
                                 mongoexport          [boolean] [default: false]

Exported options
  --all                       If all the collections of every database will be
                              exported.               [boolean] [default: false]
  --databases                 The databases that will be exported. All the
                              collections of a database will be exported.
                              Eventual exporting options passed to this option
                              will overwrite the default ones. Note: on the cli,
                              only strings are allowed.                 [string]
  --collections               The collections that will be exported. Eventual
                              exporting options passed to this option will
                              overwrite the default ones and the ones in the
                              "database" option. Note: on the cli, only strings
                              are allowed.                 [array] [default: []]
  --systemCollections         If also system collections will be exported.
                                                      [boolean] [default: false]
  --throwIfLackOfPermissions  If for permissions causes there is an error while
                              listing databases or collections of the MongoDB,
                              an error will be thrown. If the value is false,
                              the databases and collections that cannot be
                              listed will be ignored and not be exported. Note:
                              Actually all the errors that happen while listing
                              databases or collections, not only the permission
                              ones, will be thrown.   [boolean] [default: false]
  --warnIfLackOfPermissions   If for permissions causes there is an error while
                              listing databases or collections of the MongoDB, a
                              warning message will be logged. NB: Actually all
                              the errors that happen while listing databases or
                              collections, not only the permission ones, will be
                              warned.                 [boolean] [default: false]
  --throwIfOneFails           If the mongoexport of a collection fails, an error
                              will be thrown. If the  value is false, the result
                              of the function will have code PARTIAL(= 1),
                              specifying that not all the expected collections
                              were exported.          [boolean] [default: false]
  --warnIfOneFails            If the mongoexport of a collection fails, a
                              warning will be logged. [boolean] [default: false]

Exporting options
  --quiet           Runs mongoexport in a quiet mode that attempts to limit the
                    amount of output.                 [boolean] [default: false]
  --verbose         Increases the amount of internal reporting returned on
                    standard output or in log files. Increase the verbosity with
                    the -v form by including the option  multiple times, (e.g.
                    -vvvvv.). Value from 1 to 5                         [number]
  --type            Specifies the file type to export. Specify csv for CSV
                    format or json for JSON format. If you specify csv, then you
                    must also use either the --fields or the  --fieldFile option
                    to declare the fields to export from the collection.
                                               [string] [choices: "json", "csv"]
  --jsonFormat      Modifies the output to use either canonical or relaxed mode
                    of the MongoDB Extended JSON (v2) format.
                                      [string] [choices: "canonical", "relaxed"]
  --jsonArray       Modifies the output of mongoexport to write the entire
                    contents of the export  as a single JSON array. By default
                    mongoexport writes data using one JSON document for every
                    MongoDB document.                 [boolean] [default: false]
  --pretty          Outputs documents in a pretty-printed format JSON.
                                                      [boolean] [default: false]
  --query           Provides a query as a JSON document (enclosed in quotes) to
                    return matching documents in the export. You must enclose
                    the query document in single  quotes ('{ ... }') to ensure
                    that it does not interact with your shell environment.
                    Starting in MongoDB 4.2, the query must be in  Extended JSON
                    v2 format (either relaxed or canonical/strict mode),
                    including enclosing the field names and operators in quotes.
                    You can pass the argument either as a string (it will
                    automatically be included in apixes) or as an object (only
                    from config file).                                  [string]
  --fields          Specifies a field or fields to include in the export.
                                                           [array] [default: []]
  --fieldFile       An alternative to --fields. The --fieldFile option allows
                    you to specify  in a file the field or fields to include in
                    the export and is only valid  with the --type option with
                    value csv. The file must have only one field per line, and
                    the line(s) must end with the LF character (0x0A).  [string]
  --noHeaderLine    By default, mongoexport includes the exported field names as
                    the first line in a CSV output.   [boolean] [default: false]
  --skip            Use --skip to control where mongoexport begins exporting
                    documents.                                          [number]
  --limit           Specifies a maximum number of documents to include in the
                    export.                                             [number]
  --sort            You can pass the argument either as a string (it will
                    automatically be included in apixes) or as an object (only
                    config file).                                       [string]
  --forceTableScan  Forces mongoexport to scan the data store directly instead
                    of traversing the _id field index.[boolean] [default: false]
  --prependDbName   If the file name will be prepended by the database of the
                    collection. The format is: "database_filename.extension".
                                                                       [boolean]
  --fileName        A string which is the name of the file of the exported
                    collection.                                         [string]
  --filePath        A string which is the path of the file of the exported
                    collection.                                         [string]
  --absolutePath    If the filePath value is absolute and not relative to the
                    outDir option.                    [boolean] [default: false]

Log options
  --silent  If nothing will be logged.                [boolean] [default: false]
  --log     The log modes. If there is more than a mode, they must be specified
            in an array.
      [array] [choices: "base", "command", "mongoexport", "expectedCollections",
                                        "actualCollections"] [default: ["base"]]

Out options
  --outDir   The path were the exported collections will be saved.
                                                [string] [default: "./exported"]
  --outType  The type of the saving location.
                            [string] [choices: "deep", "flat"] [default: "deep"]

Options:
  --help         Show help                                             [boolean]
  --version      Show version number                                   [boolean]
  --options, -o  A path to a json config file. If an option is both on the file
                 and in the command, the command one will be considered

For more information, find our manual at
https://github.com/euberdeveloper/mongoback#readme
```
