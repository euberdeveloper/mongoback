...
async function parseIndipendentCollectionsWithDb(rootOptions: ExportingOptions, db: string, collections: ExportedIndipendentCollections, parsed: ParsedCollections): Promise<void> {
    for (const collection of collections) {
        await parseIndipendentCollection(rootOptions, db, collection, parsed);
    }
}

export async function parseCollectionsWithDatabase(rootOptions: ExportingOptions, collectionsWithDatabase: ExportedCollectionsWithDatabase[], parsed: ParsedCollections): Promise<void> {
    for (const collections of collectionsWithDatabase) {
        for (const db in collections) {
            ...
            await parseIndipendentCollectionsWithDb(exportingOptions, db, colls, parsed);
        }
    }
}