...
export function mergeOptions(options: Options = {}): Options {
    ...
    if (mergedOptions.uri) {
        ...
        if (mergedOptions.username) mergedOptions.username = encodeURIComponent(mergedOptions.username);
        if (mergedOptions.password) mergedOptions.password = encodeURIComponent(mergedOptions.password);
    }
